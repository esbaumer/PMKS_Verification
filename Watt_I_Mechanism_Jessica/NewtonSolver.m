...
for theta=1:1:10
    ...
    eqn4=momentVec(B, BCE_com, -fB)+momentVec(C, BCE_com, fC) + momentVec(E, BCE_com, fE) ==massMoIAB * AAB(theta,:); %only change the ==0 appropriately for newtons 2nd law
    %Link CD
    eqn5=-fC+fD+wCD==massCD * A_cd_com(theta,:);
    eqn6=momentVec(C, CD_com, -fC)+momentVec(D, CD_com, fD)==massMoIAB * AAB(theta,:); %only change the ==0 appropriately for newtons 2nd law
    %Link EF
    eqn7=-fE+fF+wEF==massEF * A_ef_com(theta,:);
    eqn8=momentVec(E, EF_com, -fE)+momentVec(F, EF_com, fF)==massMoIAB * AAB(theta,:); %only change the ==0 appropriately for newtons 2nd law
    %Link FG
    eqn9=-fF+fG+wFG+LoadForce==massFG * A_fg_com(theta,:);
    eqn10=momentVec(F, FG_com, -fF)+momentVec(G, FG_com, fG)+momentVec(LoadPos, FG_com, LoadForce)==massMoIAB * AAB(theta,:); %only change the ==0 appropriately for newtons 2nd law
